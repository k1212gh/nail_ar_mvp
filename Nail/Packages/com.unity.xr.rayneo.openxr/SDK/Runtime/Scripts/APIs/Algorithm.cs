...
namespace RayNeo.API
{
    ...
    {
        public static bool openDebugLog = false;
        /// <summary>
        /// 打开slam
        /// </summary>
        ...
        public static GameObject CreatePlaneMesh(XRPlaneInfo onePlaneMeshInfo, GameObject obj, bool invertYZ = false, Material m = null)
        {
            ...
            if (openDebugLog)
            {
                ...
                for (int i = 0; i < onePlaneMeshInfo.local_polygon_size; i++)
                {
                    ...
                    sb.Append(onePlaneMeshInfo.local_polygon[index]);
                    sb.Append("f,");
                    sb.Append(onePlaneMeshInfo.local_polygon[index + 1]);
                    sb.Append("f,");
                }

                sb.Append("]");
                Debug.Log("顶点信息:" + sb.ToString());
            }
            else
            {
                for (int i = 0; i < onePlaneMeshInfo.local_polygon_size; i++)
